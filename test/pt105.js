...
const SIM = function (opt) {
  ...
  for (let run = 0; run < RUNS; run++) {
    ...
    st.bossTries = Object.fromEntries(Object.entries(st.tries).filter(([k]) => /boss|hidden/.test(k)));
...
(async () => {
  ...
  rs.forEach((r, i) => {
    ...
    console.log('  Boss重试 ' + Object.values(r.bossTries).reduce((a,b)=>a+b,0) + ' 次：' + Object.entries(r.bossTries).map(([k,v])=>k+'×'+v).join(' '));
