...
const Stats = {
  ...
  foeMul(hid, tier) { return Lap.foeMulOf(tier.mul, (DB.hero(hid) || {}).q, tier.ch) * (tier.ease == null ? 1 : tier.ease); },
  ...
  stagePower(sid) {
    ...
    return (st.enemies || []).reduce((a, e) => a + this.power(this.calcEnemy(e, t.lv, t.star, this.foeMul(e, t), t.gear)), 0);
  },
};
...
const Battle = {
  ...
  putStatus(b, u, t, f) {
    ...
    const what = {
      ...
      bleed: `每回合掉血 ${num(val)}`, burn: `每回合灼伤 ${num(val)}`, poison: `每回合中毒 ${num(val)}`,
    }[f.st] || '';
    b.log.push({ c: 'st', s: `${t.ln} 【${ST_NAME[f.st] || f.st}】${what}${dur > 1 ? `，${dur} 回合` : ''}` });
  ...
  tick(b, u) {
    for (const k of Object.keys(u.status)) {
      ...
      else if (s.dur === 1 && ['stun', 'silence', 'disarm', 'chaos', 'taunt', 'vuln', 'dodge'].includes(k) && k !== 'stun' && k !== 'chaos')
